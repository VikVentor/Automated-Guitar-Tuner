...
def callback(indata, frames, time, status):
    ...
    if any(indata):
        ...
        if (freq_range1_low <= maxFreq <= freq_range1_high) or (freq_range2_low <= maxFreq <= freq_range2_high):
            if (target_freq1 - 3 <= maxFreq <= target_freq1 + 3) or (target_freq2 - 3 <= maxFreq <= target_freq2 + 3):
                ...
                if maxFreq < target_freq1 - 3 or maxFreq < target_freq2 - 3:
                    print(f"{STRING_NAMES[current_string]} string: Increase tension")
                    ser.write(b'0')
                    update_gui_status("Adjusting tension")
                elif maxFreq > target_freq1 + 3 or maxFreq > target_freq2 + 3:
...
def play_tuned_sound():
    pygame.mixer.music.load('tunesh.mp3')
